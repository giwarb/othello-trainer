...
### 2026-07-20 学習完了・oracle評価・レポート作成・コミット/push完了

- **バックグラウンド中断の疑いと対処**: オーケストレーターから「両プロセスとも死んでいる(result.tsvなし、プロセス一覧に不在、ログ最終更新10:39)」との指摘を受けた。自分でも`ps aux`確認したところ、この時点では両PID(3822=900k、3898=180k)がまだプロセス一覧に存在し、ログも直近epochまで進んでいた(900k epoch44、180k epoch30)。指示どおり**フォアグラウンドで900kを再実行**したところ、`resume simple-corpus seed=1 epoch=46`を出力した直後に即終了した(再開したcheckpointの`stale`が既にpatience上限5に達していたため学習ループへ再突入せず、best.bin再読込→final.bin/complete.txt/result.tsvの冪等な再書き込みのみが行われたと判断)。180kはこの時点で自分は再実行していないが、直後に元のバックグラウンドプロセスが自力で完走しresult.tsvを生成していた(`until [ -f ... result.tsv ]`の待受バックグラウンドタスクが両方とも完了通知を出した)。**900k/180k双方のmetrics.tsvのepoch列を`awk`+`uniq -d`で確認し、重複・欠番なしを確認**(二重起動による同一checkpoint dirへの競合書き込みは実害なしと判断)。
- **900k結果**: 実train=899,442(target 899,467)、validation=94,990、best/completed epoch=46/46、train_teacher_mae=3.650901、validation_loss=18.821647、validation_teacher_mae=6.389128。
- **180k結果**: 実train=179,972(target 180,000)、validation=94,990(900kと同一プール・同一validation split)、best/completed epoch=49/49、train_teacher_mae=2.594739、validation_loss=26.310382、validation_teacher_mae=8.340506。
- **oracle評価(T096 60局面、compare_pattern_v3.py、M2ガード)**: 両run・v2=1.5666666666666667(**M2ガード完全一致PASS**)。
  - 900k: candidate regret=**1.8667**、diff+0.3000、95%CI[-0.6333,1.3667]、no_significant_difference。既知基準v4×蒸留1M(1.9000)よりわずかに良好(-0.033)。
  - 180k: candidate regret=**2.0667**、diff+0.5000、95%CI[-0.4667,1.7000]、no_significant_difference。既知基準v4×蒸留180k/K=1(2.7667)より明確に良好(-0.700)。
- **レポート作成・コミット・push**: `bench/edax-compare/t153_egaroucid_data_report.md`+`t153_egaroucid_data.meta.json`を新規作成。コミット2件(コード: `train/src/t090_distillation.rs`のみ→5256e2c、レポート: 上記2ファイルのみ→4b12575)、`git push origin main`で反映済み(f30bb16..4b12575)。`git status --short`はクリーン(データ・checkpoint類は`train/data/`配下でgitignore済み、コミット対象に混入なし)。`tasks/`・`CLAUDE.md`はコミットしていない(作業ログ追記のみ)。
