...
2026-07-08 オーケストレーター(2回目のやり直し依頼):

前回指摘した核心的な問題(出典URLの誤り・座標の捏造疑惑)は、curl+grepによる独立検証で**完全に解消されたことを確認しました**。手順・出典URLのデータ品質は合格水準です。

ただし、`notes`欄の自由記述説明に2件、実在しない内容が見つかりました(verifierが実際にページを取得して確認済み):

1. **「馬」エントリ**: notesに「bassy84.net/zyoseki-horse.htmlは『鹿定石』『鶴定石』と並ぶ関連定石として掲載」とあるが、当該ページに「鹿」「鶴」の文字は一切存在しない。
2. **「シャープローズ定石:15-g5型」エントリ**: notesに「ページ内で著者は『Ver3』は自称であり一般名称ではないと明記している」とあるが、`post-1053.html`の本文にそのような記述は無い(別ページ(Ver2/15-b6型)からの関連記事カード抜粋にある文言を誤ってVer3自身の説明として記載してしまっている)。

対応: この2箇所の`notes`記載を、実際にページに書かれている内容に修正するか、裏付けが取れない場合は該当の説明部分を削除してください(`moves`/`sources`等の他のフィールドは変更不要です)。修正後、該当2箇所を実際に再確認した結果を作業ログに追記し、コミットしてください。今回は的を絞った小さな修正なので、迅速に対応して問題ありません。
...
2026-07-08 implementer(2回目のやり直しフィードバック対応):

**対応内容**: verifierが発見した`notes`欄の2件の事実誤認を、実際にページを再確認した上で修正した(`moves`/`sources`等は変更していない)。

1. **「馬」エントリ**: `curl`でキャッシュ済みの `bassy84.net/zyoseki-horse.html` の生HTMLに対し `grep -c "鹿\|鶴"` を実行し、**0件**であることを確認(2026-07-08実施)。指摘の通り「鹿定石」「鶴定石」への言及はこのページに存在しなかったため、notesから該当記述を削除し、混同の原因(この語は関連記事一覧ページ `othello-zyoseki-usagi-b.html` の「馬、鹿、鶴定石」というグループ見出しに由来し、それを誤って`zyoseki-horse.html`自体の内容として記載してしまっていた)を明記した訂正文に差し替えた。
2. **「シャープローズ定石:15-g5型」エントリ**: `curl`でキャッシュ済みの `bassy84.net/post-1053.html` の生HTMLに対し `grep -n "Ver3\|勝手につけました\|一般にはまかり通っていません"` を実行したところ、本文(entry-content)には該当文言が **存在しない** ことを確認(タイトル中の「Ver3」という文字列のみヒットし、自称に関する説明文はゼロ件)。該当ページの実際の本文は「ウサギ（兎）定石より、シャープローズ定石を経て、黒15手目でg5へと変化する定石。最善と言われる15手目b6と同じく【-2】評価ですね。」であり、これに差し替えた。誤りの原因も特定した: 「Ver3は当方で勝手につけました」という文言は、`zyouseki-s-rose.html`内の関連記事カード(リンク先タイトルが同一の「Ver3」記事を指す抜粋)にのみ存在しており、それを`post-1053.html`自身の本文と取り違えていた。
3. **念のための追加確認**: 同種の「Ver2は自称」という記述を使っている「Sローズ・15-b6型」エントリについても、その出典 `zyouseki-s-tamenorirose-15-b6.html` の生HTML本文(`<p>`タグ内、entry-content)を確認したところ、「ちなみに「Ver2」というのは当方で勝手につけましたので、一般にはまかり通っていません（笑）」の一文が**そのページ自身の本文に実在する**ことを確認できたため、こちらのnotesは修正不要と判断した(誤りは「シャープローズ定石:15-g5型」側のみだった)。

**検証**: 修正後 `python -m json.tool bookgen/joseki-research.json` でパースエラーなしを確認。Pythonスクリプトで `lines` 配列35件・全エントリ`sources`非空・単一ソース24件全件に「単一ソース」注記ありを再確認済み(構造面の回帰なし)。`grep -c "鹿\|鶴"` で残存箇所が1件のみ(今回追記した訂正説明文自体であり、`othello-zyoseki-usagi-b.html`由来である旨を明記した正しい記述)であることを確認。
