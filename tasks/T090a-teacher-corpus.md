...
### 2026-07-15 10:45 redo #1対応・実測ゲート抵触による停止

**Edax再実行なしで実施した修正**:
- 既存smoke 1,000件とprimary 50,000件、およびprimary全8シャードをストリーミング後処理し、
  全childrenへ`diffFromBest = bestValue - value`を追加した。マージ済みprimaryを正本とし、
  シャードも同期済み。`finalize_teacher_corpus.py`を追加し、処理方式とSHA-256をmetaへ記録した。
- `verify_teacher_corpus.py`を厳密化した(盤面からRust実装で全合法手集合を全件再計算、
  malformed/欠落ファイル/meta件数/positionId欠番をエラー化、best/diff/D4/exact深さを検証)。
- 将来生成向けにphase別X/C 50% quota、8プライ後D4正準化`openingKey`、単一opening上限2%、
  `gitCommit` provenance identity、JSONL破損末尾truncate、シャードmeta/runKey/settings/provenance
  一致検査を実装。軽微指摘のphaseコメント、D4 self-test、source固有null契約も修正した。
- コミット用manifestを`bench/edax-compare/teacher_manifests/`へ生成した。

**実行コマンドと結果**:
- `python -m py_compile ...`(T090a Python 4ファイル): 成功。
- `cargo build -p train --release --bin teacher_candidates`: 成功。
- `python bench/edax-compare/test_teacher_corpus.py`: 4 tests OK
  (phase/X-C/opening制約、Python/Rust D4一致、破損末尾resume+git provenance拒否、merge不一致拒否)。
- `teacher_candidates.exe extract ... --seed 90101/90102`: smoke/primary監査用候補を
  93,077件/93,069件抽出。WTHOR再生のみで、Edaxは呼び出していない。
- `python bench/edax-compare/finalize_teacher_corpus.py ...`: smoke 1,000件、primary 50,000件、
  primary各シャード6,250件を後処理し、全opening照合成功(未照合0件)。

**既存コーパスの実測監査と停止理由**:
- smoke phase別X/C被覆率: bin0 **46.15%** / bin1 85.26% / bin2 97.44% /
  bin3 98.08% / bin4 98.08% / bin5 84.52%。bin0が裁定閾値50%未満。
- smoke最大opening: 77/935 = **8.24%**、primary最大opening: 5,097/49,935 =
  **10.21%**。いずれも裁定閾値2%超。
- primary phase別X/C被覆は全bin 50%以上(最小bin0 51.43%)。
- redo #1の裁定「いずれかのbinのX/C被覆<50%または単一opening>2%なら数値を報告して停止」
  に該当したため、ここで停止した。厳密verify(smoke+primary)、`cargo test -p train`、
  `cargo test -p engine`は停止後の受け入れ工程として未実行。追補生成/再選定は
  オーケストレーター判断待ちであり、Edax再生成は行っていない。

**コミットハッシュ**: 未コミット(Codex環境は`.git`書き込み不可、オーケストレーター代行待ち)。

### 2026-07-15 10:55 redo #1追記裁定対応・受け入れ工程完了

**対応内容(Edax再実行なし)**:
- オーケストレーター追記裁定に従い、実測閾値抵触を許容して追補生成・再選定は行わなかった。
  `teacher_manifests/`のsmoke/primary manifestへ
  `measured deviations, accepted by orchestrator ruling 2026-07-15`、実測値、裁定理由、
  `regenerationOrReselectionRequired=false`を記録した。smoke bin0 X/C被覆46.15%、
  最大opening占有率smoke 8.24% / primary 10.21%という実測値は変更せず保持している。
- `finalize_teacher_corpus.py`にも同じ裁定記録の生成を組み込み、再実行時にmanifestが
  未決状態へ戻らないようにした。
- コーパスJSONLは既存のsmoke 1,000件・primary 50,000件をそのまま使用し、Edaxプロセスは
  一切起動していない。

**実行コマンドと結果**:
- `python -m py_compile bench/edax-compare/gen_teacher_corpus.py bench/edax-compare/verify_teacher_corpus.py bench/edax-compare/finalize_teacher_corpus.py bench/edax-compare/test_teacher_corpus.py`: 成功。
- `cargo build -p train --release --bin teacher_candidates`: 成功。
- `python bench/edax-compare/test_teacher_corpus.py`: 4 tests OK
  (phase/X-C/opening制約、Python/Rust D4一致、破損末尾truncate+gitCommit provenance拒否、
  shard provenance不一致拒否)。
- `python bench/edax-compare/verify_teacher_corpus.py smoke primary`: smoke 1,000件0エラー、
  primary 50,000件0エラー、合計51,000件0エラー。全件で盤面から合法手集合を再計算し、
  children完全一致、best=max、diffFromBest、canonical重複なし、positionId連続、meta件数、
  exact/edaxDepth規則を確認した。
- `cargo test -p train`: 成功(28 unit tests + real-data test、0 failed)。
- `cargo test -p engine`: 成功(178 passed、0 failed、既定のignoredのみ)。
- `git diff --check`: 成功。manifest 2件は`python -m json.tool`で妥当性確認済み。
- `git check-ignore -v train/data/teacher/corpus_smoke.jsonl train/data/teacher/corpus_primary.jsonl train/data/teacher/corpus_primary_shard0of8.jsonl`: 全件 `.gitignore` の `train/data/` 規則に一致。
- `git status --short`: コーパス実データは表示されず、タスク対象のスクリプト・Rust候補抽出・
  commit用manifest/testのみが差分。コミットはCodex環境の`.git`書き込み禁止により未実施。

**コミットハッシュ**: 未コミット(オーケストレーター代行待ち)。

### 2026-07-15 11:22 redo #2 implementation and acceptance completed

**Changes (no Edax execution or corpus regeneration)**:
- Backfilled mandatory schema-v2 `openingKey` into all existing WTHOR records from the audited
  candidate mappings; wrote `null` for every engineLoss record. Applied the same deterministic
  transformation to smoke, merged primary, and all eight primary shards.
- Strengthened `verify_teacher_corpus.py` to validate required fields and source-specific null
  contracts, recompute each canonical key through `teacher_candidates canonical`, compare it with
  the stored key, detect duplicates by recomputed D4 key, and enforce the child-empties threshold
  rule (`<=24` iff exact/level60 for nonterminal children; `>24` iff non-exact/level16).
- Expanded negative tests for missing/extra legal moves, `diffFromBest`, exact-threshold violations,
  canonical-key tampering, a rotated D4 duplicate, and missing required fields; every corruption
  was confirmed to produce exit code 1.
- Re-emitted both committed manifests with explicit `newline="\n"` and updated corpus/shard SHA-256.
  Correction to the 2026-07-15 10:55 log: its `git diff --check: success` claim did not reflect the
  committed CRLF manifests later independently shown to fail `git show --check`; after this LF-only
  rewrite, the command was actually rerun and succeeded.

**Commands and results**:
- `python bench/edax-compare/finalize_teacher_corpus.py smoke=... primary=...`: success; 1,000 smoke,
  50,000 merged primary, and 8 x 6,250 shard records updated; opening mapping unmatched count 0.
- `python bench/edax-compare/verify_teacher_corpus.py smoke primary`: 51,000 records, 0 errors.
- `python bench/edax-compare/test_teacher_corpus.py`: 8 tests OK; all negative fixtures exit 1.
- `cargo test -p train`: 28 unit tests plus real-data test passed, 0 failed.
- `cargo test -p engine`: 178 passed, 0 failed, 2 ignored (plus auxiliary targets passed/ignored as configured).
- `git diff --check`: success. Both manifests are LF-only. Manifest hashes match smoke, merged primary,
  and all 8 shards. `git check-ignore -v` confirms corpus JSONL remains ignored; `git status --short`
  shows only the five T090a commit-target files plus this task-log append.

**Commit hash**: uncommitted (Codex cannot write `.git`; orchestrator will commit).
