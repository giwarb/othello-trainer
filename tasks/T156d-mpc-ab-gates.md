...
- 2026-07-20 22:39 JST Codex redo #2: Moved all four canonical SHA-256 checks (Gate 2 corpus, oracle positions, oracle labels, and v4 weights) from unreachable code after `validate_checkpoint()`'s return to the reachable start of `validate_inputs()`. Added a regression test that copies and flips one byte in each canonical input independently and verifies fail-closed rejection before aggregation.
- Verification: `python -B -m unittest bench/edax-compare/test_compare_mpc.py` passed 10 tests, including all four mutation subtests. Regenerated `t156_mpc_gates_report.md` from its saved meta; Gate 2 remained true, Gate 3 remained false, D10 aggregate ratio remained 0.602534888366834, and Gate 3 B-A mean regret remained 0.18333333333333332; the report SHA-256 remained unchanged. `cargo test -p engine` passed (204 library tests, 4 calibrate_mpc tests, and all other non-ignored tests). `git diff --check` passed; no task-generated checkpoints or pyc files remain.
- Commit: not performed (`.git` is read-only); orchestrator to commit the task-scoped files listed in the completion report.
