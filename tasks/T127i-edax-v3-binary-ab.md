...
- 2026-07-18(本実行完了後): 216 run(2帯×18組×2rep×3アーム)完了(実測1,197秒≒約20分、生成プロセスと並走・無干渉)。値一致: exact帯・level16帯ともv3はbaseと全件一致(score/bestMove不一致0)。base-n2はexact帯で全件一致だがlevel16帯で1,528件中67件score不一致(最大4石差)・同一arm2回でも764件中12件非決定性を検出(スモークで見つかった懸念どおり)。速度(帯別ペア幾何平均): v3 exact 1.1281x/level16 1.2262x、base-n2 exact 1.0968x/level16 1.6073x(ただし値不一致のため不採用)。
- 集計中にサンプル抽出コードのバグを発見: 「生成済みpositionId除外」の正規表現に`re.match`を使ったため`{`始まりのJSONL行にマッチせず、除外が機能していなかった(実質全量から抽出、144親中50親が実際は生成済みと重複)。**この重複は値/速度比較の結論には影響しない**(全アームとも同一組に対し都度独立にEdaxを実行し既存corpusレコードは一切参照していないため)が、外挿用の「現在の残件・帯別内訳」は別途`re.search`版で8 shard checkpointを再スキャンして算出し直した(正しい値: exact帯未生成378,007件、level16帯未生成129,570件、既存の申し送り「exact帯が律速」と整合)。
- レポート`bench/edax-compare/t127i_edax_v3_ab_report.md`を作成。推奨: v3バイナリは値全帯全件一致・速度は両帯とも10%以上高速(exact 11.35%短縮、level16 18.45%短縮)で判定式を満たし採用提示。`-n 2`はlevel16帯で不採用、exact帯は値・決定性は満たすがv3単独より速度メリットが小さく(8.83%<11.35%)追加採用の積極的理由なしと判定。外挿: v3両帯適用で残り507,577件に対し約3.21時間短縮(24.26時間→21.05時間)。
- `python -m pytest bench/edax-compare/ -q` 68件全パス(最終確認)。`vs_edax.py`(`_edax_solve_batch`に`edax_exe`加算引数)・`test_teacher_corpus.py`(`VsEdaxSolveBatchCommandTests`追加)・`t127i_edax_v3_ab_report.md`をパス明示で`bench:`プレフィックス・`(T127i)`付きコミット予定(`tasks/`はコミットしない)。生成中プロセス(python3.11×9+wEdax群)・`gen_teacher_corpus.py`・plan/checkpoint/シャードファイルには最後まで一切触れていない。
