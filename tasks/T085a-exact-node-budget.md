...
### 2026-07-14 Codex redo #1

- `eval_cli best` / `budget-regression` に比較専用 `--exact-quota-percent 25|40|60|75` を追加し、同一releaseバイナリ・空き19〜24固定18局面・depth 10・240,000 nodes・pattern_v2・Edax 4.6 level 16同一root子評価で4候補を比較した。生データは `bench/edax-compare/t085_exact_quota_comparison.json` に保存。目的関数順の結果は以下。
  - 25%: static-only 0/18、平均regret 1.667石、exact完走3/18、平均完成深さ9.278、平均198,874 nodes。
  - 40%: static-only 0/18、平均regret 1.667石、exact完走9/18、平均完成深さ9.222、平均214,969 nodes。
  - 60%: static-only 0/18、平均regret 2.111石、exact完走13/18、平均完成深さ8.889、平均232,059 nodes。
  - 75%: static-only 0/18、平均regret 2.222石、exact完走14/18、平均完成深さ8.500、平均237,915 nodes。
  - 全候補のstatic-onlyが同率0、25/40%のregretも同率最小だったため、次順位のexact完走数が多い40%を採用。既定値を40%、policy versionを`t085a-v2`に更新した。
- 固定コーパスを48局面へ拡張。空き13〜18は各4局面を `eval_cli gen --min-empties E --max-empties E --count 4 --seed (85100+E)`、空き25〜30は各1局面を同じ規則(count 1)で生成し、既存の空き19〜24 18局面と合わせて空き13〜30を連続被覆した。生成コマンド・各帯の用途をmanifestへ記録。`budget-regression`の局面行にempties/root・leaf exact試行/完走/quota中断を追加し、13〜14のroot exact試行、15〜18の推定コストゲート、25〜30のroot exact抑制を観測可能にした。
- 木内部ExactQuota後の`fallbackReason`規則を「探索全体を止めたGlobalNodeLimit/WallClockを優先し、それらが無く実exact quota中断があればExactQuota」と明文化・実装した。
- 固定局面exact-15-2で「木内部exact開始→ExactQuota中断→同一depth=2反復を中盤探索として完走→純中盤探索とmove/score/depth一致→不完全exact rootをExact TTへ格納せず、完成中盤値をMidgame TTへ格納」を直接assertする専用テストを追加した。
- 採用40%の空き19〜24性能: 平均regret 5.56→1.667石(70.0%減)、loss>=4石 13→5件(61.5%減)。序盤75+中盤150=225局面の合算は2.20→1.947石(0.253石改善)。内訳は序盤1.88→2.133(+0.253、0.5石未満)、中盤2.36→1.853(-0.507)。
- `cargo test -p engine`: lib 151件中149 passed / 0 failed / 2 ignored、bin 4 passed、その他失敗なし。専用テストの最終assert強化後に当該テストを再実行しpass。
- `cargo build --release -p engine --bin eval_cli`: pass。
- `eval_cli budget-regression --manifest bench/edax-compare/t085_exact_positions.json --max-nodes 240000 --time-ms 1500 --exact-from-empties 18 --pattern-weights train/weights/pattern_v2.bin`: 48/48決定的一致、`exactQuotaPercent=40`、`nullMoveWithLegal=0`、`staticOnly=0`、`budgetOvershootMax=1`。
- `cargo test -p engine --release --test ffo_bench -- --nocapture`: #40〜44全問pass。nodesは42,133,930 / 192,558,694 / 266,858,189 / 384,365,162 / 413,186,354、合計1,299,102,329でredo前記録と完全一致。
- `git diff --check`: pass。コミットハッシュ: 未コミット(Codexワーカーのためオーケストレーター代行待ち)。
