...
### 2026-07-22 独立検証(verifier)

対象コミット `92341ca`(抽出・サンプル)+ `097f488`(照合完了)。コード修正なし、
`bench/logistello/verify-results/t192-checkpoint.json`(gitignore済みローカルデータ、
implementer実行時のまま残存)の独立再集計と、リポジトリ・CI状態の確認により判定した。

1. **チェックポイント再集計**: `t192-checkpoint.json`の`rows`(100件)をPythonで独自に
   集計。`wldMatch=true` 99/100、`scoreMatch=true` 62/100、`nativeCompleted=true` 100/100。
   カテゴリ別内訳も再現: black_win 39件(WLD 39/39・スコア18/39)、draw 19件
   (WLD 19/19・スコア19/19)、black_loss 42件(WLD 41/42・スコア25/42)。
   `t192_verification_report.md`§5.2の表・`verify_summary.json`と完全一致。→**再現成功**。

2. **WLD不一致1件の特定**: `t192-logistello-025`(category=black_loss、
   expectedScoreSideToMove=-8/loss、nativeScore=+6/win、差分+14で不一致38件中最大)。
   レポート§5.3の原因分析(FFO#40-49既存回帰が系統的バグの反証、draw 19/19完全一致・
   WLD符号99%一致が探索/評価/符号変換の健全性の裏付け、当該ラインの非最適継続と判断)
   は妥当と判定する。出典(ダウンロードページ)の主張は「24空きでWLD保証」であり、
   本タスクの照合はその主張自体を否定するものではない(1/100の外れ値であり、
   むしろ99%一致は出典の主張と整合的)。「不一致1件が出典主張の反例か」という問いには、
   レポートは明示的に「1/100の外れ値であり全体傾向を覆すものではない」と回答しており、
   `theoretical_score`ラベルの解釈(WLDオラクルとしての信頼性)への回答として妥当。
   厳密には出典の「全ライン24空きWLD検証済み」の主張と1件の不一致がある以上、
   「99%の実証的裏付け」であって「100%の追試証明」ではない点は留意点として付記するが、
   レポートもこれを62%スコア一致とあわせて「WLDオラクルとして有望・石差の厳密回帰には
   不向き」という結論に反映しており、過大な主張はしていない。

3. **石差62%一致の解釈整合性**: スコア不一致38件のうち37件は符号(手番視点の
   win/loss/draw)がexpectedと一致(=WLDは保たれたまま石差のみ±2〜±8のズレ)、
   符号も異なるのは前項の`t192-logistello-025`(唯一のWLD不一致)のみ、という
   独自集計結果を得た。これはレポート§5.3の主張(「不一致38件のうち37件は
   ±2〜±8の小さなズレに収まる」「唯一の例外がWLDも反転する」)と完全に整合する。
   ダウンロードページの「WLD保証・厳密スコアは1-2手先で確定」という記載とも
   整合的な実データ像である。→**整合**。

4. `cargo test -p train` を実行し全108+4(logistello_extract)+その他バイナリテスト含め
   **全件パス**(failed 0)を確認。`logistello_wld_sample_positions.json`の
   sha256を独自算出し`154561052a172a864be9646b2e1c08b073e398557ba408266774105b59143d23`
   （チェックポイント`positionsSha256`・`verify_summary.json`と一致)を確認。
   `logistello_wld_sample_labels.json`の`metadata.verification`は
   `status/tool/wldMatchRate(0.99)/scoreExactMatchRate(0.62)/各種note`を含む
   オブジェクトに更新されており、実測結果を正しく反映している(labelsファイル自体は
   検証後に再生成されているため現在のsha256はチェックポイント記録時点の
   `labelsSha256`とは一致しないが、これは想定どおりの挙動でありpositionsの不変性の方が
   照合結果の同一性担保として重要、report記載の「同一シードでpositions.jsonは不変」も
   diffで確認可能な状態)。

5. `git show --stat 92341ca` / `git show --stat 097f488` を確認。両コミットとも
   `bench/logistello/`配下のスクリプト・curated JSON・レポート・`.gitignore`・
   `train/src/bin/logistello_extract.rs`のみで構成され、`logbook.wtb`等の生データ・
   全件抽出JSONの混入なし。

6. `git status --short` はクリーン(出力なし)。`bench/logistello/data/`
   `bench/logistello/verify-results/` `bench/logistello/__pycache__/`はいずれも
   `.gitignore`で捕捉されローカルに残存しているが追跡対象外(想定どおり)。

7. `download-logbook.ps1`をコードレビュー(実DLは実施せず、指示どおり省略)。
   固定URLから`Invoke-WebRequest -OutFile`(既存ファイルを上書き)→
   `GzipStream`で展開(`File]::Create`で対象ファイルを毎回新規作成・上書き)という
   構造で、既存ディレクトリ・既存ファイルの存在によるエラー分岐がなく再実行しても
   常に上書き成功する設計。`New-Item -Force`でディレクトリ作成も冪等。
   スクリプト冒頭コメントの「再実行安全、都度公式サイトから取り直す」という
   自己申告と実装は一致していると判定する。

**GitHub Actions**: `gh run list --branch main`で`097f488`(Rust Tests)と、
その後のタスクログコミット`fc0298d`(Rust Tests / Deploy to GitHub Pages)がいずれも
`completed / success`であることを確認。

**判定: 合格。** 受け入れ基準7項目すべて満たしている。実装者の報告(WLD一致99%・
石差一致62%・原因分析・resume動作・コミット構成)は独立再集計・コードレビューの
いずれとも矛盾なく、過大な主張もない。
