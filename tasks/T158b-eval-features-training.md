...
- 2026-07-21 01:22:43 +09:00 — redo #1: raw full metrics から Gate 3 を空き数61段で再集計。seed別最大悪化は seed 1 empty=43 `+0.228951`、seed 2 empty=49 `+0.012201`、seed 3 empty=46 `+0.140260`、3seed平均最大は empty=54 `+0.059454`。2026-07-21 の事後裁定（3seed平均×空き数、上限 `+0.10`）により Gate 3 合格を維持し、seed別悪化上位を T158c 申し送りとしてレポートした。
- 再現: `bench/edax-compare/t158b_analyze.py` を追加し、stage帯加重集計、seed別・3seed平均の61段集計、NumPy PCG64固定seedの100,000回 paired bootstrap、SHA-256、Gate 2/3判定、report/meta生成を決定化。`python bench/edax-compare/t158b_analyze.py --check` PASS、`python -m json.tool bench/edax-compare/t158b_training_report.meta.json` PASS、`cargo test -p train` PASS（lib 91件を含む全test）。engine側・学習成果物は未変更、再学習なし。コミットはオーケストレーター代行待ち。
