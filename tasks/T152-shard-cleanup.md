...
status: done # 2026-07-20。シャード32本+移行バックアップ3dir=約3.81GiB解放。各段階で削除前後SHA実測一致、git追跡変更ゼロ。軽量検収(オーケストレーター検分)
...
### 2026-07-20 追加削除(オーケストレーター指示によるバックアップ3ディレクトリの削除)

上記4.で報告した判断保留分について、オーケストレーターより「同種のシャードコピーであり生成完走・全件検証済みの現在は不要」との判断・削除指示を受けたため、同じ安全手順で削除した。

**1. 削除前の安全確認**
- merged本体 SHA-256(再実測): `067a4e3a0076d39f793164c0b2168375a5a9d450cf1f7325ba0e4661e4741e86`(タスク指定値と一致、削除前と不変)
- `git check-ignore -v` で3ディレクトリ配下のファイルがいずれも `.gitignore:32:train/data/` によりgitignore領域であることを確認

**2. 削除対象ディレクトリの内容一覧(ファイル数・合計サイズ、`find -type f` + `stat -c%s` で集計)**

| ディレクトリ | ファイル数 | 合計サイズ(bytes) |
|---|---|---|
| train/data/teacher/backup-t127h-migration/ | 26 | 935,451,290 |
| train/data/teacher/backup-t127j-migration/ | 16 | 911,715,010 |
| train/data/teacher/backup-t127j-plan/ | 10 | 427,218,585 |
| **合計** | **52** | **2,274,384,885**(約2.12 GiB) |

**3. 削除**
- `rm -rf train/data/teacher/backup-t127h-migration train/data/teacher/backup-t127j-migration train/data/teacher/backup-t127j-plan` で3ディレクトリを削除(exit 0)。`ls train/data/teacher/ | grep backup` で削除確認(該当なし)。
- `corpus_expanded200k*` 系・`corpus_expanded1m.meta.json`・selection plan・`bench/edax-compare/teacher_manifests/` には一切触れていない。

**4. 削除後の再確認**
- merged本体 SHA-256(再実測): `067a4e3a0076d39f793164c0b2168375a5a9d450cf1f7325ba0e4661e4741e86`(不変)
- merged本体サイズ: 1,595,551,517 bytes(不変)
- `git status --short`: 出力なし(クリーン)

**追加解放サイズ**: 2,274,384,885 bytes(約2.12 GiB)
**累計解放サイズ(本タスク全体)**: 1,813,486,333 + 2,274,384,885 = 4,087,871,218 bytes(約3.81 GiB)

実行コマンド: `sha256sum`(削除前後)、`git check-ignore -v`、`find`/`stat`(内容一覧集計)、`rm -rf`(3ディレクトリ)、`ls`、`git status --short`。すべて成功。
