...
- 本番デプロイ確認: `git push`後、`gh run watch`でGitHub Actionsのデプロイ完了を確認し、上記と同じPlaywrightスクリプトを本番URL(`https://giwarb.github.io/othello-trainer/`)に対して再実行し、同様に全項目OKであることを確認(詳細は追記参照)。
