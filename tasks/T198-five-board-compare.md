...
### 2026-07-23 検収(verifier)

対象コミット: `cc3d83b`(push済み・CI成功済み)。実装者の完了レポートを事実確認する形で受け入れ基準を独立に再実行した。

**受け入れ基準の結果**

1. `cd app && npx vitest run` → **パス**。103 test files / 870 tests 全件成功(実装者報告と一致)。
2. **regression-catching追試** → **検知成功**。`app/src/midgame/TwoPlyCompare.tsx`の`onePlyPanelProps(...)`呼び出し2箇所(実際に打った手/最善手)で、渡すmover引数を`opponentSide`→`mover`(自分側)に一時改変(手番取り違えを模した変異)。この状態で`npx vitest run src/midgame/TwoPlyCompare.test.tsx`を実行したところ、テスト「1手先パネルの`MoveEvalOverlay`は相手番(mover=opponent)で描画される」が`expected 'black' to be 'white'`で**FAIL**することを確認。直後に`git diff`で元の`opponentSide`へ復元(`git status --short`で差分ゼロを確認)、再実行で7 tests全件パスに戻ることを確認した。テストは1手先パネルの手番取り違えを確実に検知できる。
3. `git show cc3d83b --stat` → **パス**。変更ファイルは`app/src/analysis/BlunderPanel.tsx`・`app/src/midgame/PracticeMode.tsx`・`app/src/midgame/TwoPlyCompare.css`・`app/src/midgame/TwoPlyCompare.test.tsx`・`app/src/midgame/TwoPlyCompare.tsx`・`app/src/midgame/twoPlyCompare.test.ts`・`app/src/midgame/twoPlyCompare.ts`の7件のみ、すべて`app/`配下。
4. `npm run build` → **パス**。wasmビルド込みで成功(`dist/`一式出力、`inject-sw-version`まで完走)。
5. GitHub Pages実機確認 → **パス**(Playwright CLIスクリプトで自動化、本セッションにBrowser paneツールが無いため代替実施)。
   - 中盤練習: 第1ステージで盤面ピクセルサンプリングにより合法手マスを検出しクリック→「最善ではありません」パネルが表示され、`.two-ply-compare__board-col`が5件、ラベルは`["元局面","実際に打った手(b3): 1手先(相手番)","実際に打った手: 2手先(あなたの番)","最善手(f4): 1手先(相手番)","最善手: 2手先(あなたの番)"]`、ヘッダは5件とも「打てる場所: N か所」形式、バッジは自分4件・相手2件、合法手評価セル37件。スクリーンショットで元局面(自分視点の評価オーバーレイ)・1手先(相手視点)・2手先(自分視点)の色分けと「自分」(青)「相手」(赤)バッジを目視確認。モバイル幅(375px)で`overflowingCount: 0`・`bodyOverflow: false`、縦積みレイアウトを確認。
   - 棋譜解析: 「盤面で並べる」タブで14手を実プレイ(合法手ピクセル検出→クリックを繰り返し、毎手盤面が実際に進行することを確認)し「解析開始」。解析完了後、悪手ボタン(`.analysis-result__movelist-blunder-button`、6件検出)の1件目をクリックしBlunderPanelを開くと、同じ5盤面構成(`board-col`5件、自分4/相手2バッジ、ラベル`["元局面","実際に打った手(e6): 1手先(相手番)","実際に打った手: 2手先(あなたの番)","最善手(c4): 1手先(相手番)","最善手: 2手先(あなたの番)"]`)が表示された。モバイル幅(375px)でも`overflowingCount: 0`、モーダル内で縦積み表示。
   - 使用スクリプト・スクリーンショットはセッションのscratchpad配下(`verify_pages.mjs`/`verify_analysis.mjs`、`midgame-5board-desktop.png`等)に保存、リポジトリには一切書き込んでいない。
6. Actions実行確認・`git status --short`残骸なし → **パス**。`gh run view 29964878114`でbuild/deployとも✓(成功)。`git log -1 --oneline main`と`origin/main`が一致(`443439d`、T198を`review`へ更新するタスクログコミット)。検証全体を通じて`git status --short`は常時クリーン(regression-catching追試の改変も復元済み)。

**追加確認**: `npm run build`実行時にwasmビルド・vite build・node-budget/pattern_v6のwasmチェック・sw.jsのCACHE_VERSION注入まで全ステップ成功。

**総合判定: 合格**。実装者の完了報告(870テスト・build成功・Pages実機確認・codex-review合格ゼロ指摘)を独立に再現・裏付けた。regression-catching追試でも1手先パネルの手番取り違えをテストが確実に検知することを確認できた。
