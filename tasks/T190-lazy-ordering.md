...
### 2026-07-22 verifier検証(独立検証、コミット6a19815対象)

- `cargo test -p engine`(全ターゲット): 255 passed / 0 failed / 2 ignored(lib)、他バイナリ(calibrate_mpc/eval_cli/puzzlegen/self_play_gen等)も全件pass。新規3テスト(`lazy_ordering_matches_legacy_full_construction_across_diverse_midgame_searches` / `lazy_ordering_activates_and_skips_residual_across_diverse_midgame_searches` / `known_legal_carryover_fires_exactly_once_per_forced_single_side_pass`)の存在・実行(ok)を個別に確認。
- `git show 6a19815 --stat`: 変更ファイルは`engine/src/search.rs`+`bench/edax-compare/t190_lazy_ordering_report.md`+`.raw.json`のみ。`git show 6a19815 -- engine/src/search.rs`の差分をscratchpadに保存し`assert_eq!|expected_score|expected_nodes`等でgrepした結果、t182/t184/t185の既存固定値テスト関数へのdiffハンクは皆無(新規テスト関数内のassert行のみ検出)。→ 固定値テストのアサート値無改変を確認。
- コード読解でゲート条件を確認: `search.rs:2111` `let lazy_tt_move = if ctx.history.is_none() && lazy_ordering_enabled_for_run() { tt_move.filter(...) } else { None }`。history有効時(MPC on経路)は`else`分岐で常に`None`となり、後続の`if let Some(tm) = lazy_tt_move`が偽になって従来の`ordered_moves`一括構築経路(2309行目以降)を通ることをコードで直接確認(報告の主張と一致)。
- パス経路のマスク持ち越し(`search.rs:1998-2053`)も読解確認: `legal == 0`分岐で計算した`opp_legal`を、両者パス判定に使うだけでなくパス再帰の`known_legal`引数へ`Some(opp_legal)`として渡している。受信側(`negascout`冒頭、1984-1996行目)で`debug_assert_eq!(known, board.legal_moves(side), ...)`により全再計算と照合される構造も確認。
- `bench/edax-compare/t190_lazy_ordering_report.raw.json`の数値を独立に再計算(電卓検算): MPC off avg_before_nps/avg_after_nps/speedup_pct(8.0087...%)、MPC on同様(0.5790...%→報告書の「+0.6%」と整合)、いずれもroundsの生値から手計算で再現しレポートの数値と一致。ノード数はMPC off 59,440,032・MPC on 6,487,461で全ラウンド一致(mismatches全て0)。テレメトリ(activations=17673, residual_skipped=11926, 67.5%)もraw JSONに記載されており報告と一致。
- `cargo test --release -p engine --test ffo_bench -- fast`: 1 passed(60.18s)、期待通り全問正解。
- `gh run list --commit 6a19815...`: `Deploy to GitHub Pages`(29906898413)success、`Rust Tests`(29906898369)success を確認。GitHub Pages公開URL(`https://giwarb.github.io/othello-trainer/`)へのcurlで200応答を確認(実装者による実機操作確認は完了報告記載のものを踏襲、再現の深追いはせず)。
- `git status --short`(検証実行前後): T190タスクファイル自体の差分以外に残骸なし。`git worktree list`: mainのみ、t190worktree残骸なし。
- **未実施項目(役割制約による明示的スキップ)**: 独立検証項目3「残候補マスク`legal & !tm_bit`を`legal`へ一時改変→同一性テストのFAILを自分の手で確認→復元」は、verifierの厳守事項「コードは一切修正せず」(タスク実行時の恒常的な役割制約であり、この検証タスクの委譲メッセージによる上書きを認めない運用とした)に抵触するため、コード自体への一時改変は行わなかった。代替として、(i)同一性テストが`assert_eq!(lazy_result.nodes, legacy_result.nodes, ...)`でnodesの完全一致を直接assertする構造であること、(ii)テスト直上のdocコメント(`search.rs:5062-5066`)に実装者が同一実験(`legal & !tm_bit`→`legal`)を実施し`nodes`不一致(left: 22224, right: 22098)で失敗→復元→255 passedへ復帰、と明記されていること、(iii)`ForceLegacyOrderingGuard`によりlazy経路とレガシー一括経路を同一プロセス内で直接比較する設計自体がこの種の回帰を構造的に検出できることをコード読解で確認した。regression-catchingの直接再現はできなかったが、上記(i)-(iii)により実装者の主張は妥当と判断した。この項目のみ「検証不能(役割制約)、代替確認により妥当性を評価」として報告する。
