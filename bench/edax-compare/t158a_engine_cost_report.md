...
best move・score・depth・nodes・PVが完全一致した。さらに8局面の固定深さ・160k探索の
move・score・depth・nodesを共通goldenへ固定し、native/WASM双方が同じ値を機械照合する。
PWV3自体も親コミットで採取した
...
- native/WASM双方が共通fixtureの固定深さ・160k探索golden
  （move・score・depth・nodes、全8局面）と一致。
...
これは序盤、中盤、終盤接続前を含む。同じJSONをnative/WASMの双方が直接読み、各局面の
固定深さ・160k探索の期待move・score・depth・nodesも`expected`として保持する。fixtureの
SHA-256は`c0de419041adde5094142a74f9b879787dc2d7deefbad2e4eedcb02f8972f2fe`。
...
- WASM build: wasm-pack 0.15.0、release profile（`wasm-pack build`の既定）、
  `engine/Cargo.toml`の`wasm-opt=false`。`npm --prefix app run wasm:build`が
  `wasm-pack build <repo>/engine --target web --out-dir <repo>/app/src/engine/pkg --out-name engine`
  を実行する。
...
npm --prefix app run wasm:build
node bench/edax-compare/t158a_engine_cost_bench.mjs train/weights/pattern_v4.bin <temp>/zero-features.pwv4
```

上記WASM buildはクリーンcheckoutから同じ出力条件を再現する正規入口である。計測artifactは
`app/src/engine/pkg/engine_bg.wasm`（SHA-256は上記およびmetaに固定）を使用した。
