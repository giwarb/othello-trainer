...
# redo#1(2026-07-18)で判明した既知の罠: 切替後の再実行は絶対に行わないこと

オーケストレーターが本スクリプトの`--apply`を実行し2026-07-18に実地切替が
完了・生成が再開した後、走行中の`gen_teacher_corpus.py`の
`TeacherCorpusCheckpoint._write_meta`(checkpoint書き込みのたびにmeta docを
作り直す実装)が、本スクリプトが書いた未知のトップレベルキー`edaxExeBoundary`
を毎回の書き込みで捨てることが判明した(`edaxExe`設定自体は`settings`経由で
`_expanded1m_settings_and_meta`が再生成し続けるため残るが、`edaxExeBoundary`
だけが消える)。**この結果、方式境界(切替時点の各シャード生成済み件数)は
ライブmetaから既に消滅しており、正しい値は本スクリプトの再計算では絶対に
復元できない**(生成が続いた現在の件数で計算すると誤った境界を書いてしまう)。

正しい境界値は`bench/edax-compare/teacher_manifests/
corpus_expanded1m_method_boundaries.json`(サイドカー、コミット対象=git管理
下でchekpoint書き込みの影響を受けない)に固定値として保存済み。このため
`migrate_shard()`は、meta の`settings`に既に`edaxExe`があるのに
`edaxExeBoundary`トップレベルキーが無い(=切替後に生成が再開しboundaryが
既に失われた状態)を検出すると、境界を再計算せず即座に`RuntimeError`で
拒否する(dry-run/`--apply`どちらでも)。本スクリプトは**もう再実行しない**
(実地切替は完了済み)。将来同種の切替を行う場合は、この教訓を踏まえ
`edaxExeBoundary`のようなmigration専用キーをchekpointの`_write_meta`が
保持し続ける設計に倒すか、最初からサイドカーファイル方式を使うこと。

# 実行方法(リポジトリルートから、切替前の一回限りの想定)
    ...
    # 切替後(settings.edaxExeは既にあるがedaxExeBoundaryが失われた状態)に
    # 誤って再実行すると、境界を再計算せずRuntimeErrorで拒否される
    # (`corpus_expanded1m_method_boundaries.json`を参照するよう案内される)。
"""
...
METHOD_BOUNDARIES_SIDECAR_RELPATH = "bench/edax-compare/teacher_manifests/corpus_expanded1m_method_boundaries.json"
...
def refuse_post_switch_rerun(shard_index: int, existing_meta_doc: dict, old_settings: dict) -> None:
    """redo#1(2026-07-18)対応: 切替後に生成が再開しboundaryが失われた状態での
    再実行を検出し、境界を再計算せず即座に拒否する。

    `settings.edaxExe`が既に付いているのに、トップレベルの`edaxExeBoundary`が
    無い場合、これは「本スクリプトの`--apply`実行後、生成が再開して
    `TeacherCorpusCheckpoint._write_meta`がチェックポイント書き込みのたびに
    meta docを作り直し、未知のトップレベルキー`edaxExeBoundary`を捨てた」状態
    である可能性が高い(実際に2026-07-18の実地切替で全8シャードにこの事象が
    発生したことを確認済み)。この状態で`migrate_shard`を再実行すると、現在
    (=切替後、生成が進んだ後)のjsonlレコード数を「切替時点」の値として誤って
    再計算してしまう。正しい境界値は`METHOD_BOUNDARIES_SIDECAR_RELPATH`の
    サイドカーファイルに保存済みのため、そちらを参照させ、ここでは書き込み
    (dry-run/`--apply`いずれも)を一切行わずエラー停止する。"""
    if "edaxExe" in old_settings and existing_meta_doc.get("edaxExeBoundary") is None:
        raise RuntimeError(
            f"shard {shard_index}: settings.edaxExe={old_settings.get('edaxExe')!r} is already set but "
            "no top-level edaxExeBoundary is present in this shard's meta.json. This matches the known "
            "post-switch pattern where TeacherCorpusCheckpoint._write_meta drops unknown top-level keys "
            "on every checkpoint write after generation resumed, so edaxExeBoundary was silently lost "
            "(the edaxExe switch itself and all records remain intact). Re-running this migration now "
            "would recompute a WRONG boundary from the current (post-switch, generation has continued) "
            "record count. This script refuses to proceed (no dry-run report, no --apply write). The "
            f"authoritative switch-time boundary values are recorded in {METHOD_BOUNDARIES_SIDECAR_RELPATH} "
            "instead; refer to that sidecar file, do not re-run migrate_t127j_v3_binary.py for this shard."
        )
...
def migrate_shard(shard_index: int, plan_meta: dict, apply: bool) -> dict:
    ...
    refuse_post_switch_rerun(shard_index, existing_meta_doc, old_settings)
