...
- **Part 1(MPC積極化)**: 事前登録スクリーニング(proxy)でt=1.0が規準(一致率低下≤2pp∧誤差増≤+0.05石)を満たす最も積極的な値として選定された。確認対局30局(vs Edax lv12、T175 P1と同一開幕の前半15ペア、**redo#1で`exact_from_empties`の条件交絡〔candidateが既定18のまま実行され baseline=16と不一致だった〕を訂正し再実行**)で**強度の大きな悪化は確認されなかった**(開幕単位平均差+2.17石、95%CI[-1.00, +5.33]、T175 P1(t=1.5)比で悪化どころか点推定はやや上振れ、ノイズの範囲)。訂正後は壁時計でも1手あたり+15.75%短縮・1局あたり+13.62%短縮という速くなる方向の結果になった。固定局面での制御実験(depth 12固定、60局面)で**t=1.0はt=1.5よりさらに21%のノード削減**(t=1.5: MPC off比34.3%→t=1.0: 27.1%)を確認。**t=1.0への切り替えを提案する**(採否はユーザー/オーケストレーター判断)。
...
**redo#1: 条件交絡の訂正(2026-07-21 verifier検出)**: 初回実行では`vs_edax.py`呼び出しに`--engine-exact-from-empties 16`を渡し忘れており、candidate(t=1.0)が既定値の`exact_from_empties=18`で実行されていた(baseline=T175 P1は16)。「tだけを変えた対照実験」の前提が崩れていたため、verifierが不合格と判定した。さらに作業ログ・本レポート・metaの3箇所が(実データの18ではなく)意図値の「16」のまま記載されており、レポートの正確性も欠いていた。**対応**: 旧candidateの生JSON/レポートは`-ef18`サフィックスを付けて退避(`bench/edax-compare/endgame-results/t176-p1-t10-vs-edax-lv12-results-ef18.json`等、いずれもgitignore対象でローカルのみ)し、`--engine-exact-from-empties 16`を明示して確認対局30局を再実行した(以下の数値はすべて訂正後の実データ)。あわせて`t176_confirmation_compare.py`にbaseline/candidateのsettings主要キー(depth・exact_from_empties・time_ms・max_nodes・quota・unlimited_exact_empties・tt_mb・weights・weightsSha256)の一致を検証し不一致なら例外で拒否するガード(`validate_settings_match`、自己テスト`--self-test`付き)を追加し、以後同種の交絡を静かに見逃さないようにした。

**強度**(`t176_confirmation_compare.py`、T175 P1の同じ15開幕部分集合とpaired比較、settings一致ガード通過済み):

| 集計単位 | n | 平均差(candidate-baseline) | 95%CI | 符号検定p |
|---|---:|---:|---|---:|
| 開幕単位 | 15 | +2.1667石 | [-1.0000, +5.3333] | 0.1967 |
| 局単位 | 30 | +2.1667石 | [-0.9667, +5.6667] | 0.3938 |

判定基準(「大きな悪化(平均-2石超かつCI全体マイナス)がないこと」): 平均差は正(悪化どころか改善方向)、CI上限はプラス側 → **大きな悪化なし(基準内)**。

**速度**: 壁時計(elapsedMs/wallClockSec)はbaseline(T175 P1)とcandidate(本タスク)が別々のプロセス実行(実行時刻・マシン負荷が異なりうる)のため、参考値に留めるが、条件交絡を訂正した今回は**1手あたり1741.3ms→1467.1ms(+15.75%短縮)、1局あたり51.26s→44.27s(+13.62%短縮)**と、明確に速くなる方向の結果になった。1手あたりノード数(baseline 9,570,877→candidate 9,812,831、+2.53%)は依然としてゲーム木が両アームで着手選択の時点から分岐していく〔t=1.0とt=1.5で局面ごとの選択が変わりうる〕ため信頼できる比較にはならないが、**訂正前(ef=18交絡時)の+26.01%という大きな異常値からほぼゼロ近傍(+2.53%、ノイズと見分けがつかない水準)まで縮小した**——条件交絡が異常値の主因だったことを裏付ける結果である。決定的で比較に適した速度指標は、下記1-4の制御実験(同一局面集合、t以外は完全固定、この交絡の影響を受けない独立した確認)。

異常チェック: クラッシュ・非合法手0件(stderr空)、node-budget決定性regression PASSED(10/10)、`engine_exact_from_empties: 16`・`engine_mpc_margin_t: 1.0`がsettings/runKeyに記録済み(weights SHA `e69f3b1c...`と一致、pattern_v6.binを正しく使用したことを確認)。
